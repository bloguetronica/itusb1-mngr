...
// Definitions
const int TR_TIMEOUT = 100;  // Transfer timeout in milliseconds
...
void ITUSB1Device::configureSPIMode(uint8_t channel, bool csmode, uint8_t cfrq, bool cpol, bool cpha, int &errcnt, QString &errstr) const  // Configures the given SPI channel in respect to its chip select mode, clock frequency, polarity and phase
{
    unsigned char control_buf_out[2] = {
        channel,                                                                   // Selected channel
        static_cast<uint8_t>(cpha << 5 | cpol << 4 | csmode << 3 | (0x07 & cfrq))  // Control word (specified chip select mode, clock frequency, polarity and phase)
    };
    ...
    {
        errcnt += 1;
        errstr.append("Failed control transfer (0x40, 0x31).\n");
    }
}

void ITUSB1Device::disableCS(uint8_t channel, int &errcnt, QString &errstr) const  // Disables the chip select corresponding to the target channel
{
    unsigned char control_buf_out[2] = {
        channel,  // Selected channel
    ...
    {
        errcnt += 1;
        errstr.append("Failed control transfer (0x40, 0x25).\n");
    }
}

void ITUSB1Device::disableSPIDelays(uint8_t channel, int &errcnt, QString &errstr) const  // Disables all SPI delays for a given channel
{
    ...
    {
        errcnt += 1;
        errstr.append("Failed control transfer (0x40, 0x33).\n");
    }
}

uint16_t ITUSB1Device::getCurrent(int &errcnt, QString &errstr) const  // Gets the raw value, corresponding to the measured current, from the LTC2312 ADC
{
    ...
    {
        errcnt += 1;
        errstr.append("Failed bulk OUT transfer to endpoint 1 (address 0x01).\n");
    }
    else if (libusb_bulk_transfer(handle_, 0x82, read_input_buf, sizeof(read_input_buf), &bytes_read, TR_TIMEOUT) != 0)
    {
        errcnt += 1;
        errstr.append("Failed bulk IN transfer from endpoint 2 (address 0x82).\n");
    }
    return static_cast<uint16_t>(read_input_buf[0] << 4 | read_input_buf[1] >> 4);
}

bool ITUSB1Device::getGPIO1(int &errcnt, QString &errstr) const  // Gets the current value of the GPIO.1 pin on the CP2130
{
...
bool ITUSB1Device::getGPIO3(int &errcnt, QString &errstr) const  // Gets the current value of the GPIO.3 pin on the CP2130
{
    unsigned char control_buf_in[2];
    if (libusb_control_transfer(handle_, 0xC0, 0x20, 0x0000, 0x0000, control_buf_in, sizeof(control_buf_in), TR_TIMEOUT) != sizeof(control_buf_in))
    {
...
uint8_t ITUSB1Device::getMinorRelease(int &errcnt, QString &errstr) const  // Gets the minor release version from the CP2130
{
    unsigned char control_buf_in[9];
    if (libusb_control_transfer(handle_, 0xC0, 0x60, 0x0000, 0x0000, control_buf_in, sizeof(control_buf_in), TR_TIMEOUT) != sizeof(control_buf_in))
    {
...
bool ITUSB1Device::isOpen() const  // Checks if the device is open
{
...
void ITUSB1Device::reset(int &errcnt, QString &errstr) const  // Issues a reset to the CP2130, which in effect resets the entire device
{
    ...
    {
        errcnt += 1;
        errstr.append("Failed control transfer (0x40, 0x10).\n");
    }
}

void ITUSB1Device::selectCS(uint8_t channel, int &errcnt, QString &errstr) const  // Enables the chip select of the target channel, disabling any others
{
    unsigned char control_buf_out[2] = {
        channel,  // Selected channel
...
void ITUSB1Device::setGPIO1(bool value, int &errcnt, QString &errstr) const  // Sets the GPIO.1 pin on the CP2130 to a given value
{
    ...
    {
        errcnt += 1;
        errstr.append("Failed control transfer (0x40, 0x21).\n");
    }
}

void ITUSB1Device::setGPIO2(bool value, int &errcnt, QString &errstr) const  // Sets the GPIO.2 pin on the CP2130 to a given value
{
    unsigned char control_buf_out[4] = {
...
void ITUSB1Device::close()  // Closes the device safely, if open
{
...
int ITUSB1Device::open(const QString &serial)  // Opens the device having the given serial number, and assigns its handle
{
    int retval = 0;
    if (!deviceOpen_)  // Just in case the calling algorithm tries to open a device that was already sucessfully open, or tries to open different devices concurrently, all while using (or referencing to) the same object
    {
        ...
        {
            handle_ = libusb_open_device_with_vid_pid_serial(context_, 0x10C4, 0x8C96, reinterpret_cast<unsigned char *>(serial.toLocal8Bit().data()));
...
QStringList listDevices(int &errcnt, QString &errstr)  // Non-member helper function to list devices
{
    QStringList devices;
    ...
    {
        errcnt += 1;
        errstr.append("Could not initialize libusb.\n");
    }
    else  // If libusb is initialized
    {
        libusb_device **devs;
        ...
        {
            errcnt += 1;
            errstr.append("Failed to retrieve a list of devices.\n");
        }
        else
        {
